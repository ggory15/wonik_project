#include "wonik_local_planner/WonikLocalPlanner.h"
...
PLUGINLIB_EXPORT_CLASS(wonik_local_planner::WonikLocalPlanner, nav_core::BaseLocalPlanner)
...
WonikLocalPlanner::WonikLocalPlanner()
{
}
...
bool WonikLocalPlanner::computeVelocityCommands(geometry_msgs::Twist& cmd_vel)
{
	boost::mutex::scoped_lock lock(m_odometry_mutex);

	if(!m_odometry)
	{
		ROS_WARN_NAMED("WonikLocalPlanner", "Waiting for odometry ...");
		return false;
	}
	if(m_global_plan.empty())
	{
		ROS_WARN_NAMED("WonikLocalPlanner", "Global plan is empty!");
		return false;
	}
	...
	try {
		...
		ROS_WARN_NAMED("WonikLocalPlanner", "lookupTransform(m_local_frame, m_global_frame) failed");
		return false;
	}
	...
	try {
		...
		ROS_WARN_NAMED("WonikLocalPlanner", "lookupTransform(base_link, m_global_frame) failed");
		return false;
	}
	...
	else
	{
		...
		{
			const double stop_accel = 0.5 * m_limits.acc_lim_x;
		...
		&& m_state == state_t::STATE_ROTATING && fabs(yaw_error) < M_PI / 6)
	{
		...
		ROS_WARN_NAMED("WonikLocalPlanner", "We are stuck: yaw_error=%f, obstacle_dist=%f, obstacle_cost=%f, delta_cost_x=%f",
	...
	if(m_update_counter % 20 == 0) {
		ROS_INFO_NAMED("WonikLocalPlanner", "dt=%f, pos_error=(%f, %f), yaw_error=%f, cost=%f, obstacle_dist=%f, obstacle_cost=%f, delta_cost=(%f, %f, %f), state=%d, cmd_vel=(%f, %f), cmd_yawrate=%f",
...
bool WonikLocalPlanner::isGoalReached()
{
	boost::mutex::scoped_lock lock(m_odometry_mutex);
	...
	ROS_DEBUG_NAMED("WonikLocalPlanner", "is_stopped=%d, is_reached=%d, xy_error=%f [m], yaw_error=%f [rad]",
...
bool WonikLocalPlanner::setPlan(const std::vector<geometry_msgs::PoseStamped>& plan)
{
...
void WonikLocalPlanner::initialize(std::string name, tf2_ros::Buffer* tf, costmap_2d::Costmap2DROS* costmap_ros)
{
	ros::NodeHandle nh;
	...
	m_acc_tmp = m_limits.acc_lim_x;

	m_odom_sub = nh.subscribe<nav_msgs::Odometry>("/odom", 1, boost::bind(&WonikLocalPlanner::odomCallback, this, _1));

	m_local_plan_pub = private_nh.advertise<nav_msgs::Path>("local_plan", 1);

	ROS_INFO_NAMED("WonikLocalPlanner", "base_frame=%s, local_frame=%s, global_frame=%s",
			m_base_frame.c_str(), m_local_frame.c_str(), m_global_frame.c_str());
}

void WonikLocalPlanner::odomCallback(const nav_msgs::Odometry::ConstPtr& msg)
{
	boost::mutex::scoped_lock lock(m_odometry_mutex);
	m_odometry = msg;
}
