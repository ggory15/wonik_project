#include <wonik_localization/Util.h>
#include <wonik_localization/Convert.h>
#include <wonik_localization/Solver.h>
#include <wonik_localization/GridMap.h>
 ...
 *
 */
class WonikLocalizationNode {
public:
	WonikLocalizationNode()
	{
		...
		m_sub_scan_topic = m_node_handle.subscribe("/scan", 10, &WonikLocalizationNode::scan_callback, this);
		m_sub_map_topic = m_node_handle.subscribe("/map", 1, &WonikLocalizationNode::map_callback, this);
		m_sub_pose_estimate = m_node_handle.subscribe("/initialpose", 1, &WonikLocalizationNode::pose_callback, this);
		...
		m_loc_update_timer = m_node_handle.createTimer(	ros::Rate(m_loc_update_rate),
														&WonikLocalizationNode::loc_update, this);

		m_map_update_thread = std::thread(&WonikLocalizationNode::update_loop, this);
	}
...
protected:
	...
	std::vector<scan_point_t> convert_scan(const sensor_msgs::LaserScan::ConstPtr& scan, const Matrix<double, 4, 4>& odom_to_base)
	{
		...
		try {
			...
			ROS_WARN_STREAM("WonikLocalizationNode: lookupTransform(scan->header.frame_id, m_base_frame) failed: " << ex.what());
			return points;
		}
		...
		try {
			...
			ROS_WARN_STREAM("WonikLocalizationNode: lookupTransform(m_base_frame, m_odom_frame) failed: " << ex.what());
			return points;
		}
	...
	void loc_update(const ros::TimerEvent& event)
	{
		...
		{
			ROS_WARN_STREAM("WonikLocalizationNode: Number of points too low: " << points.size());
			return;
		}
		...
		if(update_counter++ % 10 == 0) {
			ROS_INFO_STREAM("WonikLocalizationNode: score=" << float(best_score) << ", grad_uvw=[" << float(grad_std_uvw[0]) << ", " << float(grad_std_uvw[1])
	...
	{
		{
			...
			if(pose->header.frame_id != m_map_frame) {
				ROS_WARN_STREAM("WonikLocalizationNode: Invalid pose estimate frame: " << pose->header.frame_id);
				return;
			}
			...
			ROS_INFO_STREAM("WonikLocalizationNode: Got new map pose estimate: x=" << map_pose.getOrigin()[0]
	...
	void map_callback(const nav_msgs::OccupancyGrid::ConstPtr& ros_map)
	{
		...
		ROS_INFO_STREAM("WonikLocalizationNode: Got new map with dimensions " << ros_map->info.width << " x " << ros_map->info.height
	...
	void update_map()
	{
		...
		ROS_DEBUG_STREAM("WonikLocalizationNode: Got new grid at offset (" << tile_x << ", " << tile_y << ") [iworld], "
	...
	void update_loop()
	{
		...
		while(ros::ok()) {
			try {
				...
				ROS_WARN_STREAM("WonikLocalizationNode: update_map() failed: " << ex.what());
			}
			rate.sleep();
		}
	}
...
{
	// initialize ROS
	ros::init(argc, argv, "wonik_localization_node");

	try {
		WonikLocalizationNode node;

		ros::spin();
	}
	catch(const std::exception& ex) {
		ROS_ERROR_STREAM("WonikLocalizationNode: " << ex.what());
		return -1;
	}
