...
class ControlSuiteShell(cmd.Cmd):
    ...
    def do_reach(self, arg):
        ...
        goal.target_pose.pose.position.x = 0.570
        ...
        goal.target_pose.pose.orientation.w = 0.697
    ...
    def do_init(self, arg):
        ...
        goal = MoveBaseGoal()
        goal.target_pose.header.frame_id = 'map'
        goal.target_pose.pose.position.x = 0.425
        goal.target_pose.pose.position.y = 1.629
        goal.target_pose.pose.position.z = 0
    ...
    def do_pregoal(self, arg):
        ...
        calc_goal = deepcopy(self.calculate_pregoal())
    ...
    def calculate_pregoal(self):
        if len(self.translation_filtered) == 3:
            ...
            mat = self.mat_from_euler(euler)
            x_offset = -0.8  # go farward is plus
            y_offset = -0.25 # left is plus
    ...
    def calculate_goal(self):
        if len(self.translation_filtered) == 3:
            ...
            euler = euler_from_quaternion([self.quaternion_filtered[0], self.quaternion_filtered[1], self.quaternion_filtered[2], self.quaternion_filtered[3]])
            mat = self.mat_from_euler(euler)
            x_offset = -0.675  # go farward is plus
            y_offset = -0.25 # left is plus
            ...
            for i in range( int(num/2) - 1):    
                if (thetas[i] > self.theta - self.theta_thres):
                    if (thetas[i] < self.theta + self.theta_thres):
                        ...
                        if (i is not int(num/2) - 1):
                            if (pos_error <self.pos_thres and math.fabs(thetas[i+1] - np.deg2rad(137.5)) < self.theta_thres ):
                                m1 = line1[1] / line1[0]
                                m2 = line2[1] / line2[0]
